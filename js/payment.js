...
// NOTE: This script runs directly (not in IIFE) to ensure immediate execution

console.log('[Payment] Script starting...');

// Helper to show error
function paymentShowError(message) {
...
function initializePaymentPage() {
  try {
    ...
    if (typeof CONFIG === 'undefined') {
      paymentShowError('Configuration not loaded.');
      return;
    }
    ...
    if (donorBtn) {
      donorBtn.addEventListener('click', () => {
        try {
          ...
          paymentShowError('Error: ' + err.message);
        }
      });
    }
    ...
    if (verifyBtn) {
      verifyBtn.addEventListener('click', () => {
        try {
          ...
          paymentShowError('Error verifying: ' + err.message);
        }
      });
    }
    ...
    document.querySelectorAll('.pay-card[data-method]').forEach(btn => {
      btn.addEventListener('click', () => {
        try {
          ...
          paymentShowError('Error selecting payment method: ' + err.message);
        }
      });
    });
    ...
    if (doneBtn) {
      doneBtn.addEventListener('click', () => {
        try {
          ...
          paymentShowError('Error completing payment: ' + err.message);
        }
      });
    }
    ...
    paymentShowError('Error initializing: ' + err.message);
  }
}
